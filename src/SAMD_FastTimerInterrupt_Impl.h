...
bool SAMDFastTimerInterrupt::setFrequency(const float& frequency, timer_callback callback)
{
  ...
  if (_timerNumber == TIMER_TC3)
  {
    ...
    ISR_SERVO_LOGDEBUG3(F("TC_Timer::startTimer _Timer = 0x"), String((uint32_t) _SAMDTimer, HEX), F(", TC3 = 0x"),
...
bool SAMDFastTimerInterrupt::setFrequency(const float& frequency, timer_callback callback)
{
  ...
  if (_timerNumber == TIMER_TC3)
  {
    ...
    ISR_SERVO_LOGDEBUG3(F("SAMDFastTimerInterrupt: F_CPU (MHz) ="), F_CPU / 1000000, F(", TIMER_HZ ="), TIMER_HZ / 1000000);
    ISR_SERVO_LOGDEBUG3(F("TC3_Timer::startTimer _Timer = 0x"), String((uint32_t) _SAMDTimer, HEX), F(", TC3 = 0x"),
                        String((uint32_t) TC3, HEX));
  ...
  else if (_timerNumber == TIMER_TCC)
  {
    ...
    ISR_SERVO_LOGDEBUG3(F("SAMDFastTimerInterrupt: F_CPU (MHz) ="), F_CPU / 1000000, F(", TIMER_HZ ="), TIMER_HZ / 1000000);
    ISR_SERVO_LOGDEBUG3(F("TCC_Timer::startTimer _Timer = 0x"), String((uint32_t) _SAMDTimer, HEX), F(", TCC0 = 0x"),
